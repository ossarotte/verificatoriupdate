...
Option Explicit
'
' ATTENTION: use windows-1252 charset for importing into mysql!!
'
...
#Const OFFICE97 = 0    ' 0=FALSE (Office95 assumed)   1=TRUE
...
Private Const CHR_REPLACEMENT As String = "_"  ' Use "" to simply eat tab, newline, carriage return in identifiers (table and field names)
Private Const EXT_REPLACEMENT As Integer = 0  ' If 1, replace all non digit / plain alpha chars in identifiers with CHR_REPLACEMENT (table and field names)
Private Const IDENT_MAX_SIZE As Integer = 64  ' Suggest 64. Max size of identifiers (table and field names)
...
#If DOCYESNO Then
    ...
    For ctableix = 0 To cdb.TableDefs.Count - 1
        ...
        (cdb.TableDefs(ctableix).Attributes And DB_HIDDENOBJECT))) = 0 Then
            ...
            ast = IIf(Mid(UCase(nam), 1, Len(nam) - 2) = UCase(cdb.TableDefs(ctableix).Name), "  ", "* ")
...
#If DOCYESNO Then
        ...
        For cfieldix = 0 To cdb.TableDefs(ctableix).Fields.Count - 1
            ...
            ast = IIf(Mid(UCase(nam), 1, Len(nam) - 2) = UCase(cdb.TableDefs(ctableix).Fields(cfieldix).Name), "  ", "* ")
            On Error Resume Next
                    ...
                    For cfieldix = 0 To crs.Fields.Count - 1
                        ...
                        Else
                            Select Case crs.Fields(cfieldix).Type
                                ...
                                'modified for italian locale
                                '----------------------------
                                
                                Case dbDouble, dbFloat, dbNumeric, dbCurrency
                                    sqlcode = sqlcode & Replace(Format(crs.Fields(cfieldix).Value, "General Number"), ",", ".")
...
Private Function conv_name(strname As String) As String
    Dim i As Integer, m As String, str As String, ch As Integer
    

    ' replace inner spaces with CHR_REPLACEMENT
    str = strname
    i = 1
    While i <= Len(str)
        m = Mid$(str, i, 1)
        ...
        If (EXT_REPLACEMENT = 1) Then
          If ((ch < 48) Or (ch > 122) Or ((57 < ch) And (ch < 65)) Or ((90 < ch) And (ch < 97))) Then
            str = Left$(str, i - 1) & CHR_REPLACEMENT & Right$(str, Len(str) - i)
                 i = i + Len(CHR_REPLACEMENT)
          Else
            i = i + 1
          End If
        Else
          Select Case Mid$(str, i, 1)
              ...
              Case Chr$(9), Chr$(10), Chr$(13) ' tab, newline, carriage return
                  str = Left$(str, i - 1) & CHR_REPLACEMENT & Right$(str, Len(str) - i)
                  i = i + Len(CHR_REPLACEMENT)
              Case Else
                  i = i + 1
